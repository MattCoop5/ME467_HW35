...
if __name__ == "__main__":
    from hazardous_warehouse_vis import configure_rn_example_layout
...
# ---------------------------------------------------------------------------
# Optional: Z3 KB sanity check (standalone, no environment)
# ---------------------------------------------------------------------------
def kb_sanity_check():
    ...
    is_safe_2_1 = z3_entails(solver, safe(2, 1))
    is_safe_1_2 = z3_entails(solver, safe(1, 2))
    ...
    is_safe_3_1 = z3_entails(solver, safe(3, 1))
    not_ok_3_1 = z3_entails(solver, Not(safe(3, 1)))

    # What does the solver conclude about (2,2)?
    conclusion_2_2_safe = z3_entails(solver, safe(2, 2))
    conclusion_2_2_not_safe = z3_entails(solver, Not(safe(2, 2)))

    # TELL the solver the percepts at (2,1) again (rumbling this time)
    solver.add(Not(creaking_at(2, 1)))
    solver.add(rumbling_at(2, 1))

    # ASK again about (3,1) and (2,2)
    is_safe_3_1_again = z3_entails(solver, safe(3, 1))
    conclusion_2_2_again_safe = z3_entails(solver, safe(2, 2))
    conclusion_2_2_again_not_safe = z3_entails(solver, Not(safe(2, 2)))

    return {
        "is_safe_2_1": is_safe_2_1,
        "is_safe_1_2": is_safe_1_2,
        "is_safe_3_1": is_safe_3_1,
        "not_ok_3_1": not_ok_3_1,
        "conclusion_2_2_safe": conclusion_2_2_safe,
        "conclusion_2_2_not_safe": conclusion_2_2_not_safe,
        "is_safe_3_1_again": is_safe_3_1_again,
        "conclusion_2_2_again_safe": conclusion_2_2_again_safe,
        "conclusion_2_2_again_not_safe": conclusion_2_2_again_not_safe,
    }
